...
(function(){
  ...
  if(!document.getElementById('authnav-css')){
    ...
    st.textContent='#authNav{position:relative;display:inline-flex;align-items:center}'
      ...
      +'#authNav .an-drop{position:absolute;top:100%;right:0;margin-top:6px;min-width:172px;background:var(--surface,#fff);border:1px solid var(--line,#eee);border-radius:12px;box-shadow:0 10px 30px rgba(0,0,0,.18);padding:6px;z-index:9999}'
  ...
  function render(){
    ...
    if(u && u.name){
      ...
      btn.addEventListener('click', function(e){
        ...
        var open=drop.hidden; drop.hidden=!open; btn.setAttribute('aria-expanded', open?'true':'false');
      });
